...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_queen and QueenAnt.is_imposter:
            return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def helper(place, hive):
            nonlocal step
            if step < min_range:  # Move forward
                step += 1
                return helper(place.entrance, hive)
            elif step > max_range or place == hive:
                return
            elif place.bees:
                return random_or_none(place.bees)
            step += 1
            return helper(place.entrance, hive)
        return helper(self.place, hive)
        # END Problem 3 and 4
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return self.is_container and not other.is_container and not self.contained_ant
        "*** YOUR CODE HERE ***"
        # END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_watersafe and insect:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while current_place is not None:
            if current_place.ant: #and current_place.ant not in self.doubled_ants:
                if current_place.ant.is_container:
                    if current_place.ant not in self.doubled_ants:
                        current_place.ant.damage *= 2
                        self.doubled_ants.append(current_place.ant)
                    if current_place.ant.contained_ant and current_place.ant.contained_ant not in self.doubled_ants:
                        current_place.ant.contained_ant.damage *= 2
                        self.doubled_ants.append(current_place.ant.contained_ant)
                elif current_place.ant not in self.doubled_ants:
                    current_place.ant.damage *= 2
                    self.doubled_ants.append(current_place.ant)
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        if not self.is_imposter and amount >= 1:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    current_action = effect(bee.action, bee)
    def action(colony):
        nonlocal duration
        if duration == 0:
            if bee.is_backward:
