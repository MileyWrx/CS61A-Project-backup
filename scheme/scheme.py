...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        eval_operand = rest.map(lambda x: scheme_eval(x, env))
        check_procedure(operator)
        return scheme_apply(operator, eval_operand, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    buffer = expressions
    while buffer.second is not nil:
        scheme_eval(buffer.first, env)
        buffer = buffer.second
    return scheme_eval(buffer.first, env)
    # END PROBLEM 8
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            index = 0
            while index < len(formals):
                Frame.define(user_frame, formals.first, vals.first)
                formals = formals.second
                vals = vals.second
            return user_frame
        else:
            raise SchemeError("SchemeError! try to enter same number of formals and vals")
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            if self.use_env:
                # add env as the last argument into list
                python_args.append(env)
            return self.fn(*python_args)
        except TypeError:
            raise SchemeError("Warning! There is a TypeError")
        # return builtin(self.name)(python_args[0])
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        user_frame = Frame.make_child_frame(self.env, self.formals, args)
        return user_frame
        # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    print("DEBUG:", bindings.first)
    check_form(bindings.first, 2, )
    if len(bindings.first) % 2 != 0:
        raise SchemeError('bad bindings list in let form')
    symbols = nil
    vals = nil
    buffer = bindings
    while buffer != nil:
        symbols = Pair(buffer.first.first, symbols)
        vals = Pair(scheme_eval(buffer.first.second.first, env), vals)
        buffer = buffer.second
        check_formals(symbols)
    child_frame = env.make_child_frame(symbols, vals)
    return child_frame
    # END PROBLEM 15
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a lexically-scoped call evaluated in environment ENV."""
        user_frame = env.make_child_frame(self.formals, args)
        return user_frame
    # END PROBLEM 16
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    user_mu_procedure = MuProcedure(formals, expressions.second)
    return user_mu_procedure
    # END PROBLEM 16
